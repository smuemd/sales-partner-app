...
const view = (function () {
  ...
  function vwNav (model, actions) {
    let defaults = [
      { name: 'Home', route: '/' },
      { name: 'Login', route: '/login' },
      { name: 'UST', route: '/item/0x4caB5660420BECAF280553b8c5634668379b81E0' },
      { name: 'EEG', route: '/item/0x9B6084a9a35Fc638A67Fd43d6c73B9325263D2f5' },
      { name: 'Stromsteuer', route: '/item/0xE4Dd81107246BC8366a28b2F346c1F3C0146526d' },
      { name: 'Konzessionsabgabe', route: '/item/0xC3d7291E403703Ae8BbF1d924C854c60B617D611' },
      { name: 'KWK Umlage', route: '/item/0xAc67A65B42186284e3c1748927Ce2590ad51390b' },
      { name: '§19 Umlage', route: '/item/0x5be5c47D592f1eaE734c2f297a396bb5DE2610d5' },
      { name: 'Logistik', route: '/item/0x6011229E95916A766DC525b68866CB082be4b252' },
      { name: 'ledger', route: '/item/0x19BF166624F485f191d82900a5B7bc22Be569895' },
      { name: 'smpc', route: '/item/0x2F516D1e3dcB330BB44c00cb919ab5081075C77E' }
    ]
  ...
  function vwPage (model, actions) {
    return [
      model.page === 'Login'
        ...
        : model.page === 'Item' ? vwItem(model, actions) : vwHome(model, actions),
      vwData(model)
    ]
  }

  function vwData (model) {
    let style = 'overflow-wrap: break-word; white-space: pre-wrap; line-height:1.5;'
    return [
      m('pre', { style: style }, model.user ? 'User credentials: ' + JSON.stringify(model.user) : 'loading...'),
      m('pre', { style: style }, model.data ? 'App data: ' + JSON.stringify(model.data) : 'loading...')
    ]
  }
  ...
  function vwItem (model, actions) {
    ...
    return [
      m('div', 'Item Page - viewing item ' + model.params.id, ' | ', vwExternalLink())
    ]
  }
...
function createActions (model, dataApi) {
  ...
  function onNavigateTo (routeName, params) {
    ...
    if (routeName === 'Item') {
      if (!model.user.token) {
        let defaultToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6ImRlbW8iLCJpYXQiOjE1MDA3MzE3Mzd9.JqgauKlkjWEO89yrBd_9D9_zu6IDw4lKWVbhjJA4eXo'
  ...
  function fetchPublicKey (extId, token) {
    return dataApi.fetchPublicKey(extId, token)
      .then(function (data) {
    ...
    return Promise.all([
      ...
      .then((data) => {
        model.data = {
          ...
          haben: data[0],
          soll: data[1],
          getSaldo: function () { return parseInt(this.haben) - parseInt(this.soll) }
        }
      })
      .catch(function (err) {
        console.error(err)
  ...
  function fetchSoll (id) {
    return dataApi.fetchAccountSollBO(id)
      .then(function (data) {
...
function createDataApi (settings, node) {
  ...
  function fetchLedgerBO () {
    return document.node.stromkonto('0x19BF166624F485f191d82900a5B7bc22Be569895')
  }

  function fetchAccountSollBO (address) {
    return fetchLedgerBO()
