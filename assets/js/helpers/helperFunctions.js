...
function readCookie (name) {
  ...
  if (s) {
    for (i = 0, s = s.split('; '); i < s.length; i++) {
      s[i] = s[i].split('=', 2)
      if (unescape(s[i][0]) == name) return unescape(s[i][1])
    }
  }
  return null
}
...
function makeCookie (name, value, p) {
  ...
  if (p) {
    for (k in p) {
      ...
      if (k == 'expires') p[k] = isNaN(p[k]) ? p[k] : reldate(p[k])
...
export { displayEuro, readCookie, makeCookie, rmCookie }
